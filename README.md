...
- **Compartimentos de Ações**: pastas virtuais na aba Ações (funciona nas três sub-abas: Combate/Exploração/Downtime). Clique no botão "Pastas" ao lado de cada botão "Criar Ação" (um por categoria — Reações, Ações Livres, Downtime, etc.) para criar/renomear/excluir compartimentos, e arraste as ações para dentro deles para organizar do seu jeito. Cada compartimento fica restrito à categoria onde foi criado — uma pasta criada em "Reações" não aparece em "Ações Livres" nem em Downtime/Exploração, mesmo com o mesmo nome. Pra tirar uma ação de um compartimento, arraste ela para qualquer espaço da lista fora das pastas. Os compartimentos são independentes dos de Inventário.
